...
const SidebarMenu: React.FC<SidebarMenuProps> = ({
  ...
  const allItems: MenuItem[] = [
    dashboardItem,
    myProfileItem,
    ...menuItems.filter(i => i.title !== "My Profile")
  ];
        ...
        if (item.isDashboard) {
          return (
            <div key={index} className="w-full mb-1">
              <button
                onClick={() => router.push(`/ModuleSales/Sales/Dashboard${userId ? `?id=${encodeURIComponent(userId)}` : ""}`)}
                className={`flex items-center w-full p-4 rounded transition-all duration-300 hover:bg-orange-500 hover:text-white hover:shadow-md active:scale-95 bg-orange-400 text-white ${collapsed ? "justify-center" : "justify-start"}`}
              >
                <Icon size={20} />
        ...
        return (
          <div key={index} className="w-full relative">
            <button
              ...
              className={`flex items-center w-full p-4 rounded transition-all duration-300 hover:bg-orange-400 hover:text-white hover:shadow-md active:scale-95 ${collapsed ? "justify-center" : ""}`}
            >
              <Icon size={20} />
