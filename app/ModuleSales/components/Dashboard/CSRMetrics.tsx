...
const SEC_KEY_MAP: Record<
    keyof typeof COLORS,
    "responseTime" | "rfqHandlingTime" | "nonRFQHandlingTime"
> = {
...
const CSRMetrics: React.FC<CSRMetricsProps> = ({ filteredAccounts }) => {
    ...
    const maxY = Math.max(
        ...data.flatMap((d) => Array.from(activeKeys).map((key) => d[key] ?? 0)),
        0
    );
    ...
    const yScale = (val: number) =>
        maxY === 0
            ...
            : height -
            margin.bottom -
            (val / maxY) * (height - margin.top - margin.bottom);
    ...
    return (
        <div className="bg-white shadow-md rounded-lg p-6 font-sans overflow-x-auto text-black">
            <div className="flex justify-between items-center mb-4 gap-2">
                <h2 className="text-sm font-bold">CSR Metrics</h2>
                <button
            ...
            {showComputation && (
                <div className="bg-gray-50 border rounded-lg p-4 mt-6 text-xs space-y-3">
                    ...
                    <ul className="list-disc ml-5 space-y-1 text-gray-700">
                        <li>
                            <b>Response Time</b> = <i>End Date − Start Date</i> (for all CSR Inquiry records)
                        </li>
                        <li>
                            <b>RFQ Handling Time</b> = Response Time (only if <code>Activity Status = Quote-Done</code>)
                        </li>
                        <li>
                            <b>Non-RFQ Handling Time</b> = Response Time (only if <code>Activity Status = Assisted</code>)
                        </li>
                        <li>
                            All times are aggregated per <b>date_created</b> and displayed in{" "}
                        ...
                        <table className="w-full text-xs table-auto">
                            <thead>
                                <tr>
                                    ...
                                    <th className="px-4 py-2 text-left">{LABELS.responseTimeSec}</th>
                                    <th className="px-4 py-2 text-left">{LABELS.rfqHandlingTimeSec}</th>
                                    <th className="px-4 py-2 text-left">{LABELS.nonRFQHandlingTimeSec}</th>
                                </tr>
                            </thead>
                            <tbody className="divide-y divide-gray-100">
                                {data.map((row, idx) => (
                                    <tr key={idx} className="hover:bg-gray-50">
                                        ...
                                        <td className="px-4 py-2">{row.responseTime}</td>
                                        <td className="px-4 py-2">{row.rfqHandlingTime}</td>
                                        <td className="px-4 py-2">{row.nonRFQHandlingTime}</td>
                                    </tr>
                                ))}
                            </tbody>
                        </table>
