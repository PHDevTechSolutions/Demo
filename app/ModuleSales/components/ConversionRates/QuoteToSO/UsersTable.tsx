...
interface Post {
  ...
  referenceid: string;
  companyname: string; // 👈 gamit lowercase
  date_created: string;
  targetquota: number;
  soamount: number;
  actualsales: number;
...
const UsersCard: React.FC<UsersCardProps> = ({ posts }) => {
  ...
  useEffect(() => {
    ...
    const grouped = filteredPosts.reduce((acc: { [key: string]: GroupedData }, post: Post) => {
      ...
      // depende sa activeTab kung Agent o Company
      ...
      if (!acc[groupKey]) {
        const daysLapsed = periodTab === "MTD" ? Math.min(today.getDate(), fixedDays) : fixedDays * 12;
        const parPercentage = periodTab === "YTD"
          ? (parPercentages[date.getMonth() + 1] || 0)
  ...
  return (
    <div className="overflow-x-auto">
      {/* Tab for Agent vs Company */}
      <div className="mb-4 border-b border-gray-200">
        <nav className="flex space-x-4">
          <button onClick={() => setActiveTab("Agent")} className={`py-2 px-4 text-xs font-medium ${activeTab === "Agent" ? "border-b-2 border-blue-500 text-blue-600" : "text-gray-500"}`}>Per Agent</button>
          <button onClick={() => setActiveTab("Company")} className={`py-2 px-4 text-xs font-medium ${activeTab === "Company" ? "border-b-2 border-blue-500 text-blue-600" : "text-gray-500"}`}>Per Company</button>
        </nav>
      </div>
      ...
      {/* Period Tab (MTD vs YTD) */}
      <div className="mb-4 border-b border-gray-200">
        <nav className="flex space-x-4">
          <button onClick={() => setPeriodTab("MTD")} className={`py-2 px-4 text-xs font-medium ${periodTab === "MTD" ? "border-b-2 border-blue-500 text-blue-600" : "text-gray-500"}`}>MTD</button>
          <button onClick={() => setPeriodTab("YTD")} className={`py-2 px-4 text-xs font-medium ${periodTab === "YTD" ? "border-b-2 border-blue-500 text-blue-600" : "text-gray-500"}`}>YTD</button>
        </nav>
      </div>
      ...
      <table className="min-w-full table-auto">
        ...
        <tbody className="divide-y divide-gray-100">
          {Object.keys(groupedData).length > 0 ? (
            ...
            <tr>
              <td colSpan={activeTab === "Agent" ? 6 : 5} className="text-center text-xs py-4 border">
