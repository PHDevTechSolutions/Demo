...
const Table: React.FC<TableProps> = ({ products, updateProduct, grandTotal }) => {
  const [discount, setDiscount] = useState<number>(0); // percentage (0–100)

  // ✅ Compute final total after percentage discount
  const discountedTotal = useMemo(() => {
    const percentage = discount > 100 ? 100 : discount; // max 100%
    const result = grandTotal - (grandTotal * percentage) / 100;
      ...
      <table className="w-full text-sm bg-white">
        ...
        <tfoot>
          <tr className="bg-gray-100 font-semibold text-xs">
            ...
            {/* Right side: Discount + Total price */}
            <td colSpan={2} className="border-b px-2 py-2 text-right space-y-2">
              <div>
                Discount (%):{" "}
                <input
                  type="number"
                  ...
                  className="w-20 border rounded px-1 text-xs text-right ml-2"
                />
              </div>
              <div>
                Total Price:{" "}
