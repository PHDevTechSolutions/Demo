      ...
      <div className="space-y-2">
        {loading ? (
          ...
          filteredTodos.map((todo) => {
            const showAsSiteVisit = todo.typeactivity === "Outbound calls" && isSiteVisitRemark(todo.remarks);
            return (
              <div
                key={todo.id}
                onClick={(e) => {
                ...
                className={`p-2 border rounded-md cursor-pointer transition-all ${todo.scheduled_status === "Done" ? "bg-green-100" : "bg-gray-50 hover:bg-gray-100"
                  }`}
              >
                <div className="flex justify-between items-start">
                  <div className="flex-1 relative">
                    <p className="text-[11px] font-semibold mb-0.5">
                      {showAsSiteVisit ? "📅 Site Visit Scheduled" : todo.typeactivity}
                    </p>
                    ...
                    {!(todo.typeactivity === "Outbound calls" && isSiteVisitRemark(todo.remarks)) && (
                      <p className="text-[9px] text-gray-500">
                        {formatToManilaTime(todo.startdate)} → {formatToManilaTime(todo.enddate)}
                      </p>
                    )}
